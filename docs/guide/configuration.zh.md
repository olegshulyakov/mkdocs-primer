...
`toc` 决定由当前页面标题生成的目录放在哪里。窗口宽到能容下第三栏时，`auto` 把它放进右侧栏，否则置于页面标题之下——那里它是读者自行展开的折叠块，而不是横在标题与首段之间的列表。`expanded` 让它始终留在正文流里、不进右侧栏，`collapsed` 把它变成读者自行展开的折叠块，`hidden`（或 `false`）则完全不渲染。
...
这两套都包含在主题中，因此这两个选项都不会添加 CDN 请求。图标 选择仅适用于主题的 HTML 控件。语言选择器始终使用 Octicon 地球仪。 Markdown 图标语法和任意第三方 故意不支持图标包。
