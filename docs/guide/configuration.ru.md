...
`toc` определяет судьбу оглавления, построенного из заголовков страницы. `auto` раскрывает его там, где в вёрстке есть место, и оставляет свёрнутым ниже брейкпоинта боковой панели, где список занял бы весь первый экран. `expanded` и `collapsed` фиксируют одно состояние; `hidden` — или `false` — убирает блок совсем.
...
`admonition` заслуживает внимания: расширение генерирует разметку и не использует CSS. сам по себе, и `@primer/css` не имеет для него никаких правил, поэтому нестилизованный увещевание – обычная неожиданность. Тема восполняет этот пробел — см. [примеры](nested/deep-page.md#admonitions).
