...
[Styling](../styling.md) рассказывает, что в неё писать: собственные переменные раскладки темы и почему переопределение цветового токена Primer в `:root` отбрасывается, а то же правило под двумя селекторами атрибутов срабатывает.
